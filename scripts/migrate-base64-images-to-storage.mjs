...
// Load env vars from repo root even if this script is executed from a subdirectory.
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const repoRoot = path.resolve(__dirname, '..');
dotenv.config({ path: path.join(repoRoot, '.env') });
// Fallback: also load from current working directory if present
dotenv.config();
...
if (!SUPABASE_SERVICE_KEY) {
  console.error('Missing SUPABASE_SERVICE_ROLE_KEY (recommended) or SUPABASE_SERVICE_KEY in environment');
  process.exit(1);
}
...
async function uploadAndGetPublicUrl({ path, dataUrl }) {
  ...
  while (true) {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        const publicUrl = await uploadAndGetPublicUrl({ path: objectPath, dataUrl: value });
        ...
        totalMigrated += 1;
        if (totalMigrated % 25 === 0) {
          console.log(`Migrated ${totalMigrated} images so far...`);
        }
      } catch (e) {
        console.error(`Failed migrating ${table}.${imageColumn} for ${idColumn}=${id}:`, e.message || e);
      }
    }
...
async function main() {
  ...
  // Registration tables (where base64 is most common historically)
  ...
  // Tournament tables (these are often URLs already, but migrate if base64 exists)
  await migrateTableColumn({
    table: 'teams',
